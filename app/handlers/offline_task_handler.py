...
def try_to_offline2115_again():
    ...
    for offline_tasks in create_offline_url_list:
        ...
        offline_success = init.require_openapi_115().offline_download_specify_path(
            offline_tasks, failed_tasks[0]["save_path"]
        )
        if offline_success:
        ...
        for task in offline_task_status:
            if task["url"] == link:
                if task["status"] == 2 and task["percentDone"] == 100:
                    ...
                    # 处理下载成功后的清理和重命名准备
                    if init.require_openapi_115().is_directory(
                        f"{save_path}/{resource_name}"
                    ):
                        # 清除垃圾文件
                        ...
                        init.require_openapi_115().create_dir_for_file(
                            f"{save_path}", "temp"
                        )
                        # 移动文件到临时目录
                        init.require_openapi_115().move_file(
                            f"{save_path}", f"{save_path}/temp"
                        )
                    ...
                    file_list = init.require_openapi_115().get_files_from_dir(
                        new_final_path
                    )
                    # 创建软链
                    from app.handlers.download_handler import (
                        create_strm_file,
...
async def view_retry_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await require_message(update).reply_text(
        retry_text, reply_markup=reply_markup, parse_mode="MarkdownV2"
    )
...
def register_offline_task_handlers(application: Any) -> None:
    ...
    application.add_handler(
        CallbackQueryHandler(handle_clear_retry_list, pattern="^(clear_all|return)$")
    )
