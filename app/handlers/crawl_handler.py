...
async def crawl_sehua(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if init.CRAWL_SEHUA_STATUS == 1:
        ...
        await require_message(update).reply_text(
            f"🕷️ 开始爬取涩花数据，日期: {require_user_data(context)['date']}，爬取完成后会发送通知，请稍后..."
        )
        from app.core.sehua_spider import sehua_spider_by_date

        thread = threading.Thread(
            target=sehua_spider_by_date, args=(require_user_data(context)["date"],)
        )
        thread.start()
        return
...
async def crawl_jav(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if init.CRAWL_JAV_STATUS == 1:
        await require_message(update).reply_text(
            "⚠️ javbee爬取任务正在进行中，请稍后再试！"
        )
        return
    else:
        init.CRAWL_JAV_STATUS = 1
        await require_message(update).reply_text(
            f"🕷️ 开始爬取javbee数据，日期: {require_user_data(context)['date']}，爬取完成后会发送通知，请稍后..."
        )
        from app.core.av_daily_update import crawl_javbee_by_date

        thread = threading.Thread(
            target=crawl_javbee_by_date, args=(require_user_data(context)["date"],)
        )
        thread.start()
        return
