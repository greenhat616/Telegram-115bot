...
async def download_image(image_url: str, save_path: str) -> tuple[bool, str]:
    ...
    try:
        ...
        try:
            ...
            def _download():
                return http_request(
                    "GET", image_url, headers=headers, cookies=cookie_jar, timeout=60
                )
            ...
            if response.status_code == 200:
                ...
                if any(
                    ...
                    if os.path.exists(final_save_path) and file_size > 0:
                        init.logger.info(
                            f"图片下载成功: {final_save_path} ({file_size} bytes)"
                        )
...
async def sehua_spider_by_date_async(date: str) -> None:
    ...
    try:
        ...
        if not browser.driver:
            add_task_to_queue(
                init.require_bot_config().allowed_user, None, f"❌ 浏览器初始化失败！"
            )
...
async def section_spider(section_name: str, date: str) -> None:
    ...
    try:
                ...
                try:
                    ...
                    if html and len(html) > 1000:  # 确保获取到完整页面
                        ...
                        if result and result.get("title"):
                            init.logger.debug(
                                f"成功解析: {result.get('title', 'Unknown')}"
                            )
        ...
        init.logger.info(
            f"本次爬取结束 - 成功: {successful_count}, 失败: {failed_count}"
        )
        # 注意：这里不关闭浏览器，保持cookie


async def parse_topic(
    section_name: str, html: str, url: str, date: str
) -> dict[str, str | None]:
    ...
    if postmessage:
        ...
        # 下载图片到本地保存到tmp
        if result["post_url"]:
            success, local_path = await download_image(
                str(result["post_url"]), f"{init.TEMP}/sehua"
            )
            if success:
...
async def get_section_update(section_name: str, date: str) -> list[str]:
    ...
    try:
        for page_num in range(1, 10):
            url = _build_full_url(
                f"forum.php?mod=forumdisplay&fid={section_id}&page={page_num}"
            )
                ...
                try:
                    ...
                    if html and len(html) > 1000:
                        ...
                        if "normalthread_" in html or "postlist" in html:
                            topics = parse_section_page(
                                html, date, page_num, section_name
                            )
                            if topics:
            ...
            if not success:
                ...
                if page_num == 1:
                    init.logger.error(
                        f"❌ [{section_name}] 分区第1页获取失败，停止当前分区爬取"
                    )
                    return []
                break
...
def parse_section_page(
    html_content: str, date: str, page_num: int, section_name: str
) -> list[str]:
    topics = []
            ...
            for attempt in range(3):
                ...
                for _ in range(15):
                    ...
                    if len(new_content) > len(content) + 200:
                        ...
                        break
                    if (
                        "tbody id=" in new_content
                        or "postlist" in new_content
                        or "normalthread_" in new_content
                        or 'class="t_f"' in new_content
                    ):
                        passed = True
                        break
                if passed:
    ...
    try:
        with SqlLiteLib() as sqlite:
            for result in results:
                ...
                if count > 0:
                    init.logger.info(
                        f"[{result.get('title')}]检测到相同磁力链接(Hash: {magnet_hash})已存在，跳过入库！"
                    )
                    continue  # 已存在，跳过

                # 判断数据完整性
                if (
                    not result.get("section_name")
                    or not result.get("av_number")
                    or not result.get("title")
                    or not result.get("magnet")
                    or not result.get("size")
                    or not result.get("movie_type")
                    or not result.get("post_url")
                    or not result.get("publish_date")
                    or not result.get("pub_url")
                    or not specify_path
                    or not result.get("image_path")
                ):
                ...
                if check_magnet(str(result.get("magnet", ""))) is False:
                    init.logger.warn(
                        f"[{result.get('magnet')}]磁力链接格式不正确，跳过入库!"
                    )
                    continue
            ...
            init.logger.info(
                f"涩花[{results[0].get('section_name')}]版块，[{results[0].get('publish_date')}]日，[{insert_count}]条数据入库成功!"
            )
    except Exception as e:
