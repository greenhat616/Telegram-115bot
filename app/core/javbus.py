...
def _get_max_subscribe() -> int:
    return (
        init.require_bot_config().rsshub.javbus.max_subscribe if init.bot_config else 0
    )
        ...
        try:
            ...
            if _get_max_subscribe() > 0:
                ...
                if total_success_count >= _get_max_subscribe():
                    init.logger.info(
                        f"已达到最大订阅数量 {_get_max_subscribe()}，停止抓取。"
                    )
                    break
            else:
                # 不阻塞下一页的抓取
                task = asyncio.create_task(
                    parse_items(sub_category, items, page, user_input, 0)
                )
                tasks.append(task)
...
async def get_content_from_rssurl(rss_url: str) -> str | None:
    try:
        ...
        response = await loop.run_in_executor(
            executor,
            lambda: http_request(
                "GET", rss_url, timeout=init.require_bot_config().rsshub.javbus.timeout
            ),
        )
...
async def download_image(
    url: str, referer: str | None = None, save_dir: str = "/tmp/javbus"
) -> str | None:
    """异步下载图片"""
    if not url:
        return None
    ...
    try:
        ...
        loop = asyncio.get_running_loop()
        response = await loop.run_in_executor(
            executor, lambda: http_request("GET", url, headers=headers, timeout=30)
        )
...
async def parse_items(
    ...
    for item in items:
        tasks.append(
            asyncio.create_task(process_single_item(sub_category, item, user_input))
        )
...
async def process_single_item(
    sub_category: str, item: dict[str, Any], user_input: str
            ...
            if description:
                ...
                if not publish_date:
                    ...
                    if date_span and date_span.parent:
                        publish_date = (
                            date_span.parent.get_text().replace("發行日期:", "").strip()
                        )
                ...
                if magnet_table:
                    first_magnet_link = magnet_table.find(
            ...
            if magnet and av_number:
                ...
                if cover_url:
                    poster_path = await download_image(
                        str(cover_url), referer=str(pub_url)
                    )
                ...
                init.logger.warn(
                    f"跳过无效资源: {title} (Magnet: {bool(magnet)}, AV: {bool(av_number)})"
                )
                return 0
...
if __name__ == "__main__":
    ...
    try:
        asyncio.run(
            rss_javbus(
                "女优", "https://rss.yhfw.fun/javbus/search/河北彩花", "河北彩花"
            )
        )
    finally:
