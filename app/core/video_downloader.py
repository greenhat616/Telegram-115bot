        ...
        try:
            ...
            async def progress_callback(current: int, total: int) -> None:
                ...
                if (now - last_update_time).total_seconds() >= 3:
                    ...
                    await self._update_status(
                        context, chat_id, message_id, text, task_id, show_cancel=True
                    )
            ...
            await self._update_status(
                context, chat_id, message_id, "🔄 正在处理文件...", task_id
            )
            ...
            await self._update_status(
                ...
                chat_id,
                message_id,
                f"☁️ 正在上传到115: {Path(final_path).name}",
                task_id,
            )
            await self._upload_to_115(
                final_path, save_path, context, chat_id, message_id, task_id
            )
    ...
    async def _upload_to_115(
        ...
        try:
            ...
            if is_upload:
                ...
                text = f"{status}\n📄 文件: {file_name}\n📂 目录: {save_dir}"
                await self._update_status(
                    context, chat_id, message_id, text, task_id, show_cancel=False
                )
            else:
                await self._update_status(
                    context,
                ...
                keyboard = [
                    [
                        InlineKeyboardButton(
                            "❌ 取消下载", callback_data=f"v_cancel_{task_id}"
                        )
                    ]
                ]
